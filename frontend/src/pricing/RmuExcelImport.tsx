// Bulk RMU price update from a spreadsheet — the same browser-side Excel round-trip as the
// Transformer/LV import: the workbook is read here and sent as plain rows; nothing is written until
// the summary is confirmed. Rows are matched on (Kind, Key) — the database's identity, not a code.
// "Cost (USD)" is the COST; the selling price the app uses = cost / factor.
...
const COLUMNS = ["Kind", "Key", "Description", "Cost (USD)", "Selling (USD)"] as const;

const flat = (s: string) => String(s ?? "").trim().toLowerCase().replace(/\s+/g, " ");

/** Column order does not matter on upload — the header text is matched. Only the COST is read; the
 *  selling column (whatever its header) is not mapped, so it never overwrites the stored cost. */
...
export function parseWorkbook(buf: ArrayBuffer): { rows: RmuImportRow[]; missing: string[] } {
  const wb = XLSX.read(buf, { type: "array" });
  const ws = wb.Sheets[wb.SheetNames[0]];
  const raw = XLSX.utils.sheet_to_json<Record<string, unknown>>(ws, { defval: null });

  const rows: RmuImportRow[] = [];
  ...
  for (const r of raw) {
    ...
    const kind = String(mapped.kind ?? "").trim();
    const key = String(mapped.key ?? "").trim();
    if (!kind || !key) continue; // no Kind + Key = no identity, skip
    rows.push({ kind, key, label: String(mapped.label ?? "").trim(), costUsd: toNum(mapped.costUsd) });
  }
  const missing: string[] = [];
  if (!seen.has("kind")) missing.push("Kind");
  if (!seen.has("key")) missing.push("Key");
  if (!seen.has("costUsd")) missing.push("Cost (USD)");
  return { rows, missing };
}
...
export default function RmuExcelImport({ onApplied }: { onApplied: () => void }) {
  ...
  const [preview, setPreview] = useState<RmuImportPreview | null>(null);
  ...
  const downloadCurrent = async () => {
    ...
    try {
      const r = await api.pricing.list();
      const factor = r.factor || 0.85;
      const out = r.rows.filter((x) => x.active);
      const header = COLUMNS as unknown as string[];
      // Cost is what we store; Selling = cost / factor, for the reader's reference only.
      const body = out.map((x) => [x.kind, x.key, x.label, x.costUsd, factor > 0 ? Math.round(x.costUsd / factor) : 0]);
      const ws = XLSX.utils.aoa_to_sheet([header, ...body]);
      ws["!cols"] = header.map((c, i) => ({
        wch: Math.max(12, Math.min(48, body.reduce((m, row) => Math.max(m, String(row[i] ?? "").length), c.length) + 2)),
      }));
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, "RMU");
      XLSX.writeFile(wb, "PowerLine RMU database (current).xlsx");
      setDone(`Downloaded ${out.length} RMU price${out.length === 1 ? "" : "s"}.`);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Could not build the current RMU sheet.");
    } finally {
      setDl("");
    }
  };
  ...
  const onFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!rows.length) { setError("No RMU rows with a Kind and Key were found in the first sheet."); return; }
      setBusy(`Checking ${rows.length} rows…`);
      const p = await api.pricing.rmuImportPreview(rows);
      setPreview(p);
  ...
  const apply = async () => {
    ...
    try {
      const r = await api.pricing.rmuImportApply(preview.batchId, includeRemovals);
      setPreview(null);
  ...
  const cancel = async () => {
    ...
    setPreview(null);
    if (id) await api.pricing.rmuImportCancel(id).catch(() => {});
  };
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 p-4" onClick={cancel}>
          <div className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-xl2 bg-white shadow-2xl dark:bg-neutral-900" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-line p-5">
              <h3 className="text-lg font-extrabold text-ink">Review RMU changes</h3>
            ...
            <div className="border-t border-line p-5">
              {s.removals > 0 && (
                <label className="mb-3 flex items-start gap-2 text-sm text-ink">
                  ...
                  <span>Remove the <b>{fmt(s.removals)}</b> RMU price{s.removals === 1 ? "" : "s"} not in this file (they are in the database but the sheet no longer lists them).</span>
                </label>
              )}
