...
export default function SendForApprovalMenu({
  ...
  // Load the approver list the first time the menu opens.
  useEffect(() => {
    if (!open || approvers) return;
    let alive = true;
    api.qtns.approvers()
      .then((r) => { if (alive) setApprovers(r.users); })
      .catch(() => { if (alive) setApprovers([]); });
    return () => { alive = false; };
  }, [open, approvers]);
  ...
  return (
    <div ref={ref} className={`relative inline-block text-left ${className ?? ""}`}>
      ...
      {open && (
        <div className="absolute right-0 z-40 mt-1 w-64 overflow-hidden rounded-xl2 border border-line bg-white shadow-lift animate-pop">
